...
- [ ] 8.1. 创建设置界面:
    - [ ] 创建一个 `SettingsScreen` Composable。
    - [ ] 添加从主界面到设置界面的导航。
- [ ] 8.2. 实现数据导入/导出:
    - [ ] 设计好用于导入/导出的 JSON 数据结构。
    - [ ] 在 `SettingsScreen` 添加“导入”和“导出”按钮。
    - [ ] 实现导出逻辑：从数据库读取所有资产和投资组合数据，序列化为 JSON，并使用 Storage Access Framework (SAF) 让用户选择保存位置。
    - [ ] 实现导入逻辑：使用 SAF 让用户选择文件，读取并解析 JSON，然后在一个数据库事务中清空现有数据并插入新数据。
- [ ] 8.3. 实现后台刷新频率设置:
    - [ ] 集成 `DataStore` 用于持久化存储用户设置。
    - [ ] 在 `SettingsScreen` 中添加一个选项，允许用户选择后台数据刷新的频率。
    - [ ] 修改里程碑 6 中的任务调度逻辑，使其能够根据用户在设置中选择的频率来动态地调度后台任务。
