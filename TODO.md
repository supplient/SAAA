...
* [ ] 将买入因子和卖出阈值的计算过程可视化，便于调试
    * [ ] 将中间计算过程存入asset中
        ...
        * [ ] 存入买入因子的中间计算过程到`AssetAnalysis`
            * 计算过程`buyFactorLog`为一个string
            * 其实质就是将计算过程编写成一个单行日志
            * 例如：`<目标占比>-<当前占比>=<相对偏移>; <相对偏移>/(<相对偏移>+<半饱和相对偏移>=<偏移因子>; max(0, -<七日涨跌幅>)=<七日绝对跌幅>; <七日绝对跌幅>/(<七日绝对跌幅>+<半饱和跌幅>)=<跌幅因子>; <偏移权重>*<偏移因子>+<1-偏移权重>*<跌幅因子>=<去波动率的买入因子>; <1-资产波动率>*<去波动率的买入因子>=<买入因子>`
        * [ ] 存入卖出阈值的中间计算过程到`AssetAnalysis`
            * `<基础卖出阈值>*<1-资产波动率>*<1-总体风险因子>=<卖出阈值>`
        * [ ] 存入总体风险因子的中间计算过程到资产配置`Portfolio`中
            * `<第一个资产风险>+<第二个资产风险>+...<最后一个资产风险>=<资产总体风险>; <资产总体风险>/(<资产总体风险>+<半饱和总体风险>)=<总体风险因子>`
    * [ ] 在`AddEditAssetScreen`的最下面增加对买入因子、卖出因子的计算过程的只读展示
        * 其中，原字符串里的"; "应当被替换为换行符
    * [ ] 在`ConfigNoteScreen`的最下面增加对总体风险因子的计算过程的只读展示
