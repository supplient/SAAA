...
* [x] 优化可用现金编辑
* [x] 资产列表界面里点击总体风险或除现金外占比时弹出资产总览
    * 资产总览：总市值、可用现金、除现金外占比、总体风险因子
* [x] 优化交易记录编辑界面的操作
...
* [x] 重写交易记录列表界面
    * 每条交易记录为三行：
        ...
        * 第二行：
            * <买/卖><份额>x<单价>+<手续费>=<+/-总额>
            * 要根据交易类型是买还是卖而变化“买/卖”、“+/-”，并变化`<买/卖><份额>`和`<+/-总额>`的颜色
        * 第三行交易理由，单行，不允许换行，较小字体
* [x] 修正从交易记录界面进入交易修改界面时数据显示不正确的问题
* [x] 让资产分析界面也显示`PortfolioSummary`，并增加控制是否显示的isHidden按钮
* [x] 默认让资产分析界面和资产列表界面的`isHidden`都为true
* [x] 导入数据时清空现有的交易记录和交易机会，并弹出提示警告用户
* [x] 将从资产列表界面`AssetListScreen`进入资产分析界面`AssetAnalysisScreen`和交易记录列表`TransactionListScreen`的入口从侧边栏改为资产列表界面顶部的两个按钮
* [x] 新增资产不再使用按钮，而是在资产列表的最底部增加一行新增按钮
* [x] 重构代码，让排序选项由列来提供，而非UI界面本身



### v0.1.0: 小修小补
修正数据获取与数据算法：
* [x] `getMarketStats`里的获取方式会不会获取不到当前价格？以及波动率计算是不是应该有不含今天的前90天？
* [x] 考虑更改波动率计算方法，改为`Yang-Zhang optimal volatility estimator`
    * 它能够把前一天的收盘价到第二天的开盘价之间的价格波动也考虑进来，还有根据最高、最低估算出的日内波动率也考虑进来

UI改进：
* [x] 排序改成通过点击表头来实现：
    * [x] 点击表头时会根据该列提供的排序方案弹出SortDialog让用户进行选择
    * [x] 移除掉排序按钮
